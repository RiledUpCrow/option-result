...
export class Option<T> {
  ...
  /**
   * Converts the Option to a Result with specified `error` if the value is
   * missing.
   */
  public okOr = <E>(error: E): Result<T, E> =>
    this.isSome() ? Result.ok(this.get()) : Result.err(error);

  /**
   * Converts the Option to a Result with error generated by specified `func`
   * callback if the value is missing.
   */
  public okOrElse = <E>(func: () => E): Result<T, E> =>
    this.isSome() ? Result.ok(this.get()) : Result.err(func());
